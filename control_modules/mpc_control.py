...
class ControlMPC(BaseController):
    ...
    #    self.handle_ready()
        self.max_power = self._heater.get_max_power() * self.const_heater_power
    ...
    def _load_profile(self):
        ...
        This helper is primarily used when the constructor is given a
        pre-built profile (used by the calibration code).  It mirrors the
        behaviour of ``_load_config_variables`` but operates on a plain
        dictionary instead of a klipper config section.
        """
    ...
    def get_profile(self):
        # gather the current configuration into a dictionary suitable for
        # passing back to the constructor or for calibration routines.  the
        # keys match those expected by ``_load_profile`` and by the
        # existing calibration logic.
        return {
            "block_heat_capacity": self.const_block_heat_capacity,
            "ambient_transfer": self.const_ambient_transfer,
            "target_reach_time": self.const_target_reach_time,
            "heater_power": self.const_heater_power,
            "smoothing": self.const_smoothing,
            "sensor_responsiveness": self.const_sensor_responsiveness,
            "min_ambient_change": self.const_min_ambient_change,
            "steady_state_rate": self.const_steady_state_rate,
            "filament_diameter": self.const_filament_diameter,
            "filament_density": self.const_filament_density,
            "filament_heat_capacity": self.const_filament_heat_capacity,
            "maximum_retract": self.const_maximum_retract,
            "filament_temp_src": self.filament_temp_src,
            "ambient_temp_sensor": self.ambient_sensor,
            "cooling_fan": self.cooling_fan,
            "fan_ambient_transfer": self.const_fan_ambient_transfer,
        }
