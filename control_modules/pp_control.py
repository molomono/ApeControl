...
class PPControl(BaseController):
    ...
    def temperature_update(self, read_time, temp, target_temp):
        ...
        if time_diff >= self.config_params.ff.min_deriv_time:
            temp_deriv = temp_diff / time_diff
        else:
            temp_deriv = (self.prev_temp_deriv * (self.config_params.ff.min_deriv_time - time_diff) + temp_diff) / self.config_params.ff.min_deriv_time
    ...
    def ff_fb_control(self, read_time):
        ...
        if z_position < 0.3:
            fist_layer_compensation = self.config_params.ff.dt_first_layer
        else:
        ...
        self.e_velocity_filtered = max(0.0, (1 - self.config_params.ff.ev_smoothing) * self.e_velocity_filtered + self.config_params.ff.ev_smoothing * e_velocity)
        # Feed forward control logic
        u_ff = (self.target_temp - fist_layer_compensation) * self.config_params.ff.k_ss + fan_speed * self.config_params.ff.k_fan + self.e_velocity_filtered * self.config_params.ff.k_ev

        
        logging.info("%s: Control Effort: FB_PWM: %.3f, FF_PWM: %.3f, FF_ev: %.3f" % (self.algo_name, u_fb_bidirection, u_ff, self.e_velocity_filtered * self.config_params.ff.k_ev))
    ...
    def _state_max_power(self, error, duration, read_time):
        ...
        if error < self.config_params.ff.t_overshoot_up:
    ...
    def _state_coast_up(self, error, duration, read_time):
        ...
        elif duration >= self.config_params.ff.coast_time_up:
            self._transition("regulate", read_time)
        return 0.0
    ...
    def _state_regulate(self, error, duration, read_time):
        ...
        if abs(error) < self.config_params.ff.t_delta_regulate or duration < self.config_params.ff.min_regulation_duration: # Temp within regulation window or min duration not met
            ...
            self._transition("max_power", read_time)
            return 1.0
        elif error < -self.config_params.ff.t_delta_regulate:  # Temp too far above target
    ...
    def _state_min_power(self, error, duration, read_time):
        ...
        if error > -self.config_params.ff.t_overshoot_down:  # Error approaching zero from below
    ...
    def _state_coast_down(self, error, duration, read_time):
        ...
        if self.prev_temp_deriv >= 0:
            self._transition("regulate", read_time)
        elif duration >= self.config_params.ff.coast_time_down:
            self._transition("regulate", read_time)
        return 1.0
